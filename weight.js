...
import { todayStr, weekStartFor } from "./items.js";
...
export async function loadAll() {
  ...
  try {
    const saved = await db.dbGet("settings", "weightUnit");
    if (saved && saved.value) weightUnit = saved.value;
  } catch (e) {}
}
...
// Keeps every rendered copy of the unit toggle (weight tab + settings tab) in sync.
function refreshUnitToggles() {
  ["wt-unit-toggle", "settings-wt-unit-toggle"].forEach((id) => {
...
export async function setUnit(unit) {
  ...
  weightUnit = unit;
  try {
    await db.dbPut("settings", { key: "weightUnit", value: unit });
  } catch (e) {}
  refreshUnitToggles();
...
window.wtSetUnit = setUnit;

export function renderSettingsUnitToggle() {
...
window.wtSave = async function () {
  ...
  if (!(await confirm_(`Save today's weight as ${displayVal} ${weightUnit}?`, "Save"))) return;
...
// ── Confirm modal (shared #confirm-modal element, same pattern as programs.js) ──
